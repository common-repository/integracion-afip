...
class CustomerInvoiceMail extends WC_Email {
	/**
	...
	public function __construct( DocumentProcessorInterface $processor ) {
		$this->id             = 'afip_order_invoice';
		$this->customer_email = true;
		$this->title          = __( 'AFIP Customer invoice', 'wc-afip' );
		$this->description    = __( 'Notification to the customer mentioning that an AFIP invoice has been created (invoice attached)', 'wc-afip' );
		$this->template_html  = Helper::locate_template( 'invoice-email.php' );
	...
	public function send_email( WC_Order $order ) {
		...
		$email_subject = Helper::get_option( 'invoice_mail_subject', 'Tu factura de {{sitio}} - Orden #{{orden}}' );
	...
	public function get_mail_content( WC_Order $order, string $email_subject ): string {
		...
		$email_body = Helper::get_option( 'invoice_mail_body', 'Aquí está tu factura de la orden {{orden}}' );
		...
		ob_start();
		require_once Helper::locate_template( 'invoice-email.php' );
